...
trait ShellString {
    fn shell_replace(&self) -> String;
}
impl ShellString for str {
    /// The function escapes any special characters and prepares the string for a shell
    fn shell_replace(&self) -> String {
...
fn json_synchronize(json_file: &mut File, shortcuts_file: &mut File, value: &Value) -> Result<()> {
    ...
    serde_json::to_writer_pretty(&mut *json_file, value)
        .context(format_err!("Could not read Value"))?;
    json_file.flush().unwrap();
    let aliases = value
        .as_object()
        .context(format_err!("Not an object"))?
        .get("aliases")
        .context("aliases key not found, run `csc setup`")?
        .as_object()
        .context(format_err!("Not an object"))?;
